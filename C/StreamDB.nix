    ...
    flake-utils.lib.eachDefaultSystem (system:
      ...
      in
      {
        packages.default = pkgs.stdenv.mkDerivation rec {
          ...
          dontUnpack = true; # Sources are generated in the derivation

          srcs = let
            # Internal header for core StreamDB (declarations only)
            streamdb_h = pkgs.writeText "streamdb.h" ''
              /*
            ...
            # Public header for the library (wrapper API)
            libstreamdb_h = pkgs.writeText "libstreamdb.h" ''
              /*
               * libstreamdb - Wrapper API for StreamDB
               * 
               * Copyright (C) 2025 DeMoD LLC
               * 
               * This library is free software; you can redistribute it and/or
              ...
              #ifndef LIBSTREAMDB_H
              #define LIBSTREAMDB_H
              ...
              char* streamdb_write_document(StreamDB* db, const char* path, const void* data, size_t size);
              char** streamdb_search(StreamDB* db, const char* prefix, size_t* count);
              void streamdb_set_quick_mode(StreamDB* db, int quick);
              int streamdb_get_async(StreamDB* db, const char* path, void* callback, void* user_data);
              int streamdb_begin_async_transaction(StreamDB* db, void* callback, void* user_data);
              int streamdb_commit_async_transaction(StreamDB* db, void* callback, void* user_data);
              int streamdb_rollback_async_transaction(StreamDB* db, void* callback, void* user_data);

              #endif
            '';

            # Core implementation (adjusted for separate header)
            streamdb_c = pkgs.writeText "streamdb.c" ''
              #include "streamdb.h"
              #include <stdio.h>
              #include <stdlib.h>
              #include <string.h>
            ...
            wrapper_c = pkgs.writeText "libstreamdb_wrapper.c" ''
            ...
            '';
          in
            pkgs.runCommand "streamdb-sources" {} ''
              mkdir -p $out
              cp ${streamdb_h} $out/streamdb.h
              cp ${libstreamdb_h} $out/libstreamdb.h
              cp ${streamdb_c} $out/streamdb.c
              cp ${wrapper_c} $out/libstreamdb_wrapper.c
            '';
          ...
          preBuild = ''
            cp ${srcs}/streamdb.h .
            cp ${srcs}/libstreamdb.h .
            cp ${srcs}/streamdb.c .
            cp ${srcs}/libstreamdb_wrapper.c .
          '';
          ...
          installPhase = ''
            ...
            cp ${srcs}/libstreamdb.h $out/include/libstreamdb.h
          '';
